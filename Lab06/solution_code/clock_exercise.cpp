...
public:
	...
	Clock( std::string time_string ) {
		...
		for(auto ch : time_string) {
			if( ch != ':' ) {
				...
				append_to_hour = false;
			}
		}
		// Convert strings to integers
		int h {0}, m {0};
		try {
			h = std::stoi(hour_string);
			m = std::stoi(minute_string);
		} catch(std::invalid_argument& e) {
			throw std::runtime_error("Invalid time");
		}
		// Check that integers are valid times
		if( h < 0 || h > 23 || m < 0 || m > 59 ) {
			throw std::runtime_error("Invalid time");
		}
		hour = h;
	...
	*/
	std::string get_time_string( bool format24 = true ) {
		...
		if( format24 ) {
			time_string += int_to_string(hour, 2);
			time_string += ":";
			time_string += int_to_string(minute, 2);
		} else {
			int h = get_hour12();
			time_string += int_to_string(h, 2);
			time_string += ":";
			time_string += int_to_string(minute, 2);
			if( is_pm() ) {
				time_string += "pm";
	...
	*/
	...
	void advance_time(int add_hour, int add_minute) {
		if( add_hour < 0 || add_hour > 23 || add_minute < 0 || add_minute > 59 ) {
			throw std::runtime_error("Invalid input to advance_time");
		}
		hour += add_hour;
		minute += add_minute;
		if( minute > 59 ) {
